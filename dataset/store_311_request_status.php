...
$join_sql = "SELECT * FROM service_category 
INNER JOIN 311_ward ON 'service_category.ticket_number = 311_ward.ticket_number' 
INNER JOIN 311_neighbourhood ON '311_ward.ticket_number = 311_neighbourhood.ticket_number' 
INNER JOIN 311_request_status ON '311_neighbourhood.ticket_number = 311_request_status.ticket_number'";
...
for($i = 1; $i < sizeof($table_array); $i++){
	...
	if ($check_result->num_rows > 0) {
		...
		$join_sql = "SELECT * FROM checked_311_result INNER JOIN " . $table_array[$i] ." 
		ON checked_311_result.ticket_number = " . $table_array[$i] .".ticket_number";
		$join_result = $conn->query($join_sql);//get returned result
