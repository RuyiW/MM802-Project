...
$table_array = Array("complaint", "bylaw_status", "month", "bylaw_year");
...
$complaint = str_replace('%20', ' ', $complaint);
//echo "in data.php";
//echo $ward;

if($checked == '1'){
	$data_sql = "SELECT * FROM Bylaw WHERE complaint = '$complaint'"; //select data that matches
	...
	if ($data_result->num_rows > 0) {
		...
		while($row = $data_result->fetch_assoc()) {
			...
			if($row["complaint"] != null){
				...
				$insert_sql = "INSERT INTO complaint (complaint_number, bylaw_year, month_number, month, report_period, bylaw_neighbourhood, 
	...
	$delete_sql = "DELETE FROM complaint WHERE complaint = '$complaint'"; //select data that matches
	if (mysqli_query($conn, $delete_sql)) {
	...
	$delete_sql = "DELETE FROM checked_bylaw_result WHERE complaint = '$complaint'"; //select data that matches
	if (mysqli_query($conn, $delete_sql)) {
    	echo "The record deleted successfully";
    	echo "\n";
	} else {
