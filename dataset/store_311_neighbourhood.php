...
if($checked == '1'){
	...
	$delete_sql = "DELETE FROM 311_neighbourhood WHERE 311_neighbourhood = '$neighbourhood'"; 
	if (mysqli_query($conn, $delete_sql)) {
	...
	$delete_sql = "DELETE FROM checked_311_result WHERE 311_neighbourhood = '$neighbourhood'"; 
	if (mysqli_query($conn, $delete_sql)) {
    	echo "The record deleted successfully";
    	echo "\n";
	} else {
...
if ($previous_count['COUNT(*)'] == 0) {
	if($checked == '1'){
		...
		if (mysqli_query($conn, $copy_sql)) {
			echo "The record copyed from temp";
			echo "\n";
		} else {
			echo "Error: " . $copy_sql . "<br>" . mysqli_error($conn);
			echo "\n";
